...
func defaultLogger() {
	...
	slog.Info("test", "a", "b", slog.String("c", "d")) // want `use consistent arguments \(either bare or Attr\)`
	slog.Info("test", slog.String("a", "b"), slog.String("c", "d"))
}

func logger(logger *slog.Logger) {
	logger.Info("test", "a", "b", "c", "d")
	logger.Info("test", "a", "b", slog.String("c", "d")) // want `use consistent arguments \(either bare or Attr\)`
